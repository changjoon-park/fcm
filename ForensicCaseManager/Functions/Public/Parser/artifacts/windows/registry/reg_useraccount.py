...
class UserAccount(ForensicArtifact):
    ...
    def calculate_samkey(self, syskey: bytes) -> bytes:
        ...
        f_reg = self.src.source.registry.key(self.SAM_KEY).value("F").value
        ...
        if f.revision not in [0x02, 0x03]:
            raise ValueError(
                f"Unsupported Domain Account F revision encountered: {f.revision}"
            )

        if fk.revision not in [0x01, 0x02]:
            raise ValueError(
                f"Unsupported SAM Key Data revision encountered: {fk.revision}"
            )

        if (
            fk.revision == 0x01
        ):  # SAM key revision 0x01 involving RC4 (samsrv.dll: KEDecryptKeyWithRC4)
            ...
            key_data = f_key[
                len(c_sam.SAM_KEY_AES) : len(c_sam.SAM_KEY_AES) + fk.data_len
            ]
            checksum_data = f_key[
                len(c_sam.SAM_KEY_AES)
                + fk.data_len : len(c_sam.SAM_KEY_AES)
                + fk.data_len
                + fk.checksum_len
            ]
            samkey = AES.new(syskey, AES.MODE_CBC, fk.salt).decrypt(key_data)[:16]
            checksum = AES.new(syskey, AES.MODE_CBC, fk.salt).decrypt(checksum_data)[
                :32
            ]
    ...
    def sam(self) -> Generator[dict, None, None]:
        """Dump SAM entries
        ...
        syskey = self.src.source.dpapi.syskey  # aka. bootkey
        ...
        for reg_path in self.iter_entry(entry_name="Users"):
            for users_key in self.src.source.registry.keys(reg_path):
                for user_key in users_key.subkeys():
                    ...
                    u_username = v_data[
                        v.username_ofs : v.username_ofs + v.username_len
                    ].decode("utf-16-le")
                    u_fullname = v_data[
                        v.fullname_ofs : v.fullname_ofs + v.fullname_len
                    ].decode("utf-16-le")
                    u_admin_comment = v_data[
                        v.admin_comment_ofs : v.admin_comment_ofs + v.admin_comment_len
                    ].decode("utf-16-le")
                    u_user_comment = v_data[
                        v.user_comment_ofs : v.user_comment_ofs + v.user_comment_len
                    ].decode("utf-16-le")

                    u_lmpw = v_data[v.lmpw_ofs : v.lmpw_ofs + v.lmpw_len]
                    u_ntpw = v_data[v.ntpw_ofs : v.ntpw_ofs + v.ntpw_len]

                    lm_hash = decrypt_single_hash(
                        f.rid, samkey, u_lmpw, almpassword
                    ).hex()
                    nt_hash = decrypt_single_hash(
