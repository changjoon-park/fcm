...
class CaseManager:
            ...
            if self._artifacts:
                for artifact_entry in self._artifacts:
                    if artifact == artifact_entry:
                        self.forensic_artifact.append(ForensicArtifact(
                            src=self.src,
                            artifact=artifact,
                            category=category
                        ))
    ...
    def _computer_name(self):
        ...
        try:
            ...
            _ = computer_name.encode("ASCII")  # ! test if the name's each character is ascii (not extended)
            return computer_name
        except:
            return convertfrom_extended_ascii(string=computer_name, encoding="UTF-16-LE")
    
    @property
    def _registered_owner(self):
        reg_path = "HKLM\\SOFTWARE\\Microsoft\\Windows NT\\CurrentVersion"
        registered_owner = self.src.source.registry.key(reg_path).value("RegisteredOwner").value
        try:
            _ = registered_owner.encode("ASCII")
            return registered_owner
        except:
            return convertfrom_extended_ascii(string=registered_owner, encoding="UTF-16-LE")
    ...
    def export_all(self) -> list[dict]:
        ...
        return self._export_session(result_files=result_files), self._export_case_information()
    ...
    def _export_case_information(self):
        case_information = "[" + json.dumps(self.case_information, indent=2, default=str, ensure_ascii=False) + "]"
...
if __name__ == '__main__':
    ...
    parser.add_argument("-l", "--local", action="store_true", help="local to parse", dest="local")
    parser.add_argument("-c", "--container", default=None, help="container to parse", dest="container")
    parser.add_argument("-a", "--artifact", default=None, help="artifact to parse", dest="artifact")
    parser.add_argument("-y", "--category", default=None, help="category to parse", dest="category")
