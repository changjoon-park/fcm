...
from util.delphi import ExtractFileName, ExtractFilePath
from util.datetime import FILETIME, FileTime, filetime_to_datetime
...
class TLNKFileHeader(LittleEndianStructure):
    _fields_ = [
        ...
        (
            "CLSID",
            TGUID,
        ),  # 클래스 식별자(class identifier;CLSID)로 항상 00021401-0000-0000-C000-000000000046 값
...
class TLNKFileParser:
    def __init__(
        self, srcfile, src_id, fileName="", fileCTime="", fileMTime="", fileATime=""
    ):
        self.src_id = src_id
        self.data = None
        if srcfile == None:
    ...
    def parse_data(self):
        ...
        hinfo.append(
            addItem(
                "HasLinkTargetIDList",
                RS_ExistsIDListInShellHeader
                if (h.LinkFlags & HasLinkTargetIDList) != 0
                else RS_NoExistsIDListInShellHeader,
                node,
            )
        )
        hinfo.append(
        ...
        if (h.LinkFlags & HasExpString) != 0:
            hinfo.append(
                addItem(
                    "HasExpString", RS_LinkSavedInEnvironmentVariableDataBlock, node
                )
            )
        if (h.LinkFlags & RunInSeparateProcess) != 0:
            hinfo.append(
                addItem("RunInSeparateProcess", RS_RunTargetInVirtualMachine, node)
            )
        ...
        if (h.LinkFlags & HasExpIcon) != 0:
            hinfo.append(
                addItem("HasExpIcon", RS_LinkSavedWithIconEnvironmentDataBlock, node)
            )
        if (h.LinkFlags & NoPidlAlias) != 0:
            hinfo.append(
                addItem(
                    "NoPidlAlias",
                    RS_FileSystemLocationRepresentedInShellNamespace,
                    node,
                )
            )
        ...
        if (h.LinkFlags & RunWithShimLayer) != 0:
            hinfo.append(
                addItem("RunWithShimLayer", RS_ShellLinkHasShimDataBlock, node)
            )
        ...
        if (h.LinkFlags & EnableTargetMetadata) != 0:
            hinfo.append(
                addItem(
                    "EnableTargetMetadata", RS_ShellLinkCollectsTargetProperties, node
                )
            )
        if (h.LinkFlags & DisableLinkPathTracking) != 0:
            hinfo.append(
                addItem(
                    ...
                    RS_EnvironmentVariableDataBlockIgnored,
                    node,
                )
            )
        if (h.LinkFlags & DisableKnownFolderTracking) != 0:
            hinfo.append(
                addItem(
                    ...
                    RS_SpecialFolderDataBlockNKnownFolderDataBlockIgnored,
                    node,
                )
            )
        if (h.LinkFlags & DisableKnownFolderAlias) != 0:
            hinfo.append(
                addItem(
                    ...
                    RS_KnownFolderDataBlock4TranslatingTargetIDList,
                    node,
                )
            )
        if (h.LinkFlags & AllowLinkToLink) != 0:
            hinfo.append(
                addItem("AllowLinkToLink", RS_CreatingLinkReferncesAnotherLink, node)
            )
        ...
        if (h.LinkFlags & PreferEnvironmentPath) != 0:
            hinfo.append(
                addItem("PreferEnvironmentPath", RS_TargetIDListShouldNotStored, node)
            )
        if (h.LinkFlags & KeepLocalIDListForUNCTarget) != 0:
            hinfo.append(
                addItem(
                    "KeepLocalIDListForUNCTarget",
                    RS_StorePropertyStoreDataBlockIfTargetUNCName,
                    node,
                )
            )
        ...
        if (h.LinkFlags & HasLinkTargetIDList) != 0:
            ...
            hinfo.append(
                addItem(
                    "Target ID List",
                    "Pos=0x%.4X, Size=%d" % (data.position - 2, targetIDListSize),
                )
            )
            node = _id
            if targetIDListSize > 0:
                ...
                while data.position < data.size:
                    ...
                    hinfo.append(
                        addItem(
                            "ID List entry - %.2d" % i,
                            "Pos=0x%.4X, Size=%d" % (data.position - 2, listLen),
                            node,
                        )
                    )
        ...
        if (h.LinkFlags & HasLinkInfo) != 0:
            ...
            node = _id
            LFLInfo = _cast(
                data.read(sizeof(TLinkFileLocationInfo)), TLinkFileLocationInfo
            )
            hinfo.append(
                addItem(
                    "Table Length",
                    "%d (%x)" % (LFLInfo.LinkInfoSize, LFLInfo.LinkInfoSize),
                    node,
                )
            )
            hinfo.append(
                addItem(
                    "Header Length",
                    "%d (%x)"
                    % (LFLInfo.LinkInfoHeaderSize, LFLInfo.LinkInfoHeaderSize),
                    node,
                )
            )
            if LFLInfo.LinkInfoFlags != 0:
                hinfo.append(
                    addItem("Link Info Flags", "Pos=0x%.4X" % data.position, node)
                )
                subNode = _id
                if (LFLInfo.LinkInfoFlags & VolumeIDAndLocalBasePath) != 0:
                    pass
                if (
                    LFLInfo.LinkInfoFlags & CommonNetworkRelativeLinkAndPathSuffix
                ) != 0:
                    pass
                else:
                ...
                if (LFLInfo.LinkInfoFlags & VolumeIDAndLocalBasePath) != 0:
                    hinfo.append(
                        addItem(
                            RS_TargetFilePathInfo, "Pos=0x%.4X" % data.position, subNode
                        )
                    )
                    subNode = _id
                    ...
                    LFLVTable = _cast(
                        data.read(sizeof(TLinkFileLocalVolumeTable)),
                        TLinkFileLocalVolumeTable,
                    )
                    hinfo.append(
                        addItem(
                            RS_VolumeName, self.getAnsiStrValue(data.position), subNode
                        )
                    )
                    volumeType = (
                        "",
                        "No root directory",
                        "Removable (Floppy, Zip, etc..)",
                        "Fixed (Hard disk)",
                    ...
                    hinfo.append(
                        addItem(
                            RS_VolumeType,
                            volumeType[LFLVTable.VolumeType]
                            if LFLVTable.VolumeType < len(volumeType)
                            else "Unknown",
                            subNode,
                        )
                    )
                ...
                if (
                    ...
                    subNode = _id
                    LFNVTable = _cast(
                        data.read(sizeof(TLinkFileNetworkVolumeTable)),
                        TLinkFileNetworkVolumeTable,
                    )
                    ...
                    if (LFNVTable.NetFlags & ValidNetType) != 0:
                        ...
                        if v == None:
                            v = "Unknown - %.8X (%d)" % (
                                LFNVTable.NetProviderType,
                                LFNVTable.NetProviderType,
                            )
                        hinfo.append(addItem("Network Provider Type", v, subNode))
                    hinfo.append(
                        addItem(
                            "Network Relative Link",
                            self.getAnsiStrValue(data.position),
                            subNode,
                        )
                    )
                    hinfo.append(
                        addItem(
                            "Net Name", self.getAnsiStrValue(data.position), subNode
                        )
                    )
                    return result
                hinfo.append(
                    addItem("Base Path", self.getAnsiStrValue(data.position), subNode)
                )
                ...
                if v != "":
                    hinfo.append(
                        addItem(
                            "Common Base Path",
                            self.getUnicodeStrValue(data.position),
                            subNode,
                        )
                    )
                if (h.LinkFlags & HasName) != 0:
                    print(data.position)
                    hinfo.append(
                        addItem("Name", self.getUnicodeStrValue(data.position), subNode)
                    )
                if (h.LinkFlags & HasRelativePath) != 0:
                    hinfo.append(
                        addItem(
                ...
                if (h.LinkFlags & HasArguments) != 0:
                    hinfo.append(
                        addItem(
                            "Arguments", self.getUnicodeStrValue(data.position), subNode
                        )
                    )
                if (h.LinkFlags & HasIconLocation) != 0:
                    hinfo.append(
        ...
        while stpos <= data.size - 10:
            ...
            if blk_sig == 0xA0000003:
                hinfo.append(
                    addItem("Distributed Tracker", "Pos=0x%.4X" % data.position)
                )
                node = _id
                ...
                if ep != -1:
                    ep += sp
                    hinfo.append(
                        addItem("Machine Id", data.data[sp:ep].decode("utf-8"), node)
                    )
    ...
    def parse(self):
        ...
        result = {
            "LinkFileInfo": [
                ["sid", "Name", "Path", "CreationTime", "ModifiedTime", "AccessTime"]
            ]
        }
