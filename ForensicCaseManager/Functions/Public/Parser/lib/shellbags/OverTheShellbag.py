...
def RefineResultToList(_parsing_result, _Bagkey):
    ...
    if mtime == "":
        mtime = fat_mtime
    bag_path = ("BagMRU" + _Bagkey).replace('/', '\\')
    last_written_time = _parsing_result["last_written_time"]
    last_written_time = last_written_time.strftime('%Y-%m-%d %H:%M:%S.%f')

    return [str(bag_path), str(shell_type), str(value), str(mtime), str(atime), str(ctime), str(
        last_written_time), str(mft_entry_number), str(mft_sequence_number), str(filesystem)]
