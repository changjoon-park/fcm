...
function Invoke-ForensicCaseManager {
    ...
    process {
        # Call Invoke-PythonScript inside a try/catch block for error handling
        try {
            Invoke-PythonScript -PythonExecutable $Python -ScriptPath $ScriptPath -Case $Case -Artifact $Artifact -Category $Category
            if ($LASTEXITCODE -ne 0) {
                throw "Invoke-PythonScript exited with code $LASTEXITCODE"
            }
        } catch {
            Write-Error "Failed to invoke ForensicCaseManager: $_"
            return
        }
    }

    end {
        Write-Output "Successfully Parsed Case: $Case"
